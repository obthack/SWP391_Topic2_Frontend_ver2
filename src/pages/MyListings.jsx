...
export const MyListings = () => {
  ...
  const loadListings = async () => {
    try {
      const data = await apiRequest(`/api/Product/seller/${user?.accountId || user?.id}`);
      setListings(data || []);
    } catch (error) {
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600 mt-2">Quản lý và theo dõi các bài đăng của bạn</p>
            </div>
            <Link
              to="/create-listing"
        ...
        {filteredListings.length === 0 ? (
          <div className="bg-white rounded-xl shadow-sm p-12 text-center">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">
              {listings.length === 0 ? 'Chưa có tin đăng nào' : 'Không tìm thấy tin đăng phù hợp'}
            </h3>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
            {filteredListings.map((listing) => (
              <div key={listing.id} className="bg-white rounded-xl shadow-sm overflow-hidden hover:shadow-md transition-shadow">
                <div className="relative">
                  <img
                    src={listing.images?.[0] || 'https://images.pexels.com/photos/110844/pexels-photo-110844.jpeg?auto=compress&cs=tinysrgb&w=400'}
                    alt={listing.title}
                ...
                <div className="p-6">
                  ...
                  <div className="flex items-center justify-between text-sm text-gray-500 mb-4">
                    <span className="flex items-center">
                      ...
                      {listing.viewsCount || listing.views_count || 0} lượt xem
                    </span>
                    <span>{formatDate(listing.createdAt || listing.created_at || listing.createdDate)}</span>
                  </div>
